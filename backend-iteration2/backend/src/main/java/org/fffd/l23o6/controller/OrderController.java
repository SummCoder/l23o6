...
public class OrderController {
    ...
    public CommonResponse<OrderIdVO> createOrder(@Valid @RequestBody CreateOrderRequest request) {
        StpUtil.checkLogin();
        return CommonResponse.success(new OrderIdVO(orderService.createOrder(StpUtil.getLoginIdAsString(), request.getTrainId(), request.getStartStationId(), request.getEndStationId(), request.getSeatType(), null, request.isWhether_use_points())));
    }
    ...
    public CommonResponse<?> patchOrder(@PathVariable("orderId") Long orderId, @Valid @RequestBody PatchOrderRequest request) {
        //前端传来状态更新，已取消或已支付状态，后端在此做相应操作
        switch (request.getStatus()) {
            case PAID:
                if (!orderService.payOrder(orderId, request.getPaymentType().getText())) return CommonResponse.error(BizError.PAY_FAILED);
                break;
            case CANCELLED:
                if (!orderService.cancelOrder(orderId)) return CommonResponse.error(BizError.CANCEL_FAILED);
                break;
            default:
