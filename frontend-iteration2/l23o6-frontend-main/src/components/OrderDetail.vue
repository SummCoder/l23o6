...
const pay = (id: number) => {
  request({
    ...
    data: {
      status: '已支付',
      payment_type: orderDetail.data.payment_type,
    }
  }).then((res) => {
    ElNotification({
      offset: 70,
...
const paymentMethods = ['微信支付', '支付宝支付']; //前端添加支付方式
...
<template>
  <div style="display: flex; flex-direction: column">
    ...
    <el-descriptions :column="4" border>
      ...
      <el-descriptions-item label="车票原价" :span="2" width="25%" align="center" v-if="orderDetail.data">
        {{ orderDetail.data.origin_amount}}
      </el-descriptions-item>
      <el-descriptions-item label="积分变化" :span="2" width="25%" align="center" v-if="orderDetail.data">
        {{ orderDetail.data.remain_points}}
      </el-descriptions-item>
      <el-descriptions-item label="支付方式" :span="2" width="25%" align="center" v-if="orderDetail.data">
        {{ orderDetail.data.payment_type}}
      </el-descriptions-item>
      <el-descriptions-item label="支付金额" :span="2" width="25%" align="center" v-if="orderDetail.data">
    ...
    <div style="margin-top: 2vh" v-if="orderDetail.data && orderDetail.data.status === '等待支付'">
      ...
      <el-form-item label="&emsp;&emsp;&emsp;&emsp;选择支付方式&emsp;&emsp;" prop="payment_type" >
        <el-radio-group v-model="orderDetail.data.payment_type">
          <el-radio v-for="method in paymentMethods" :key="method" :label="method">
            {{ method }}
          </el-radio>
        </el-radio-group>
      </el-form-item>
