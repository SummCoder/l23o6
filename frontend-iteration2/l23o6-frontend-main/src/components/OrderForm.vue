...
const orderRules = reactive<FormRules>({
  ...
  points: [{ required: true, message: '此字段为必填段', trigger: 'change'}],
  seat_type: [{ required: true, message: '此字段为必填项', trigger: 'change' }],
  whether_use_points: [{required: true, message: '此字段为必填项', trigger: 'change'}],
})
...
<template>
  ...
  <el-form ref="orderFormRef" :model="orderForm" :rules="orderRules" label-width="120px">
    ...
    <el-form-item label="积分" prop="points">
      <el-input v-model=orderForm.points disabled />
    </el-form-item>
    ...
    <el-form-item label="是否使用积分" prop="whether_use_points">
      <el-radio-group v-model="orderForm.whether_use_points">
        <el-radio :label="true" :disabled="orderForm.points<=0">是</el-radio>
